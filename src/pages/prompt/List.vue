<template>
  <q-page padding>
    <div class="row">
      <q-table title="Prompts" :rows="prompts" :columns="columnsPrompt" row-key="id" class="col-12" :loading="loading">
        <template v-slot:top>
          <span class="text-h6 text-black">
            Prompts para ChatGPT
          </span>
          <q-space />
          <q-btn class="desktop-only" label="Adicionar" color="primary" icon="mdi-plus" :to="{ name: 'form-prompt' }" />
        </template>
